...
public class Shop : MonoBehaviour
{
 ...
 /*   public void sellItem(Item finalItem)
    {
        inventoryPlayer.deleteItem(finalItem);


        if (Input.GetKey(KeyCode.LeftShift) && charMotor.isInShop)
        {
            print("selling process...");
            playerInv.goldCoins += GetComponent<ItemOnObject>().item.itemValue;
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (other.tag == "Player")
        {
            ...
            PrepareShop();
            playerinv.mainInventory.openInventory();
        }
    }
    ...
    private void OnTriggerExit(Collider other)
    {
        if (other.tag == "Player")
        {
            ...
            playerinv.mainInventory.closeInventory();
        }
    }
}