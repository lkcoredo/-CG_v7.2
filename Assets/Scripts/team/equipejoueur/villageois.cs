...
public class villageois : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        FindObjectOfType<Livings>().add_villageois();
    }
    ...
    void Update()
    {
        ...
        if (!isDead)
        {
            ...
            TargetTeam2Array = GameObject.FindGameObjectsWithTag("arbre");
            ...
            for (int i = 0; i < TargetTeam2Array.Length; i++)
            {
                ...
                // 
                if (Distance < chaseRange && Distance > attackRange && TargetTeam2Array[i].GetComponent<Arbre>().currentHealth > 0)
                {
                    animations.Play("Walk");
                ...
                // 
                if (Distance < attackRange && TargetTeam2Array[i].GetComponent<Arbre>().currentHealth > 0)
                {
                    isMoving = false;
                    ...
                    if (Time.time > attackTime)
                    {
                        ...
                        TargetTeam2Array[i].GetComponent<Arbre>().ApplyDamage(TheDamage);
                        int TheDamageInt = (int)TheDamage;
                        MesRessources.currentWood = MesRessources.currentWood + TheDamageInt;
                        Debug.Log("Votre villageois a recupéré " + TheDamage + " unités de bois");
    ...
    void patroling()
    {
        ...
        TargetRandom = new Vector3(xPos, 0, zPos);
        agent.SetDestination(TargetRandom);
    }
    ...
    public void Dead()
    {
        ...
        {
            isDead = true;
            FindObjectOfType<Livings>().remove_dead_villageois();
        }
